...
export default function App() {
  ...
  function handleSessionLogged(result) {
    // Update concept mastery in state
    setConceptsMap(prev => {
      const updated = { ...prev };
      Object.keys(updated).forEach(sid => {
        updated[sid] = updated[sid].map(c =>
          c.id === result.concept.id
            ? { ...c, mastery: result.concept.mastery, status: result.concept.status }
            : c
        );
      });
      return updated;
    ...
    if (selectedConcept?.id === result.concept.id) {
      setSelectedConcept(prev => ({ ...prev, ...result.concept }));
    }
    // Refresh stats
    api.getStats().then(res => setStats(res.stats));
    api.getWeekly().then(res => setWeekly(res.weekly));
  }
